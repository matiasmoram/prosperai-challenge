# Prosper Health voice agent — technical reference & operations manual

> **The engineering deep-dive + how-to manual.** For the non-technical / executive
> overview start with **[`SOLUTION.md`](./SOLUTION.md)**; this document is the
> section-by-section reference for *how every feature is wired* and *how to run,
> evaluate, and operate the system*. It is also the **canonical "what exists
> today" ledger** (kept in lock-step with the code — see `CLAUDE.md` rule 11).
>
> **Start with §0 (summary), §2 (quick start), §3 (quick evaluate).** Pair with
...
recorded in `CHANGELOG.md` and the relevant sections of this document.
