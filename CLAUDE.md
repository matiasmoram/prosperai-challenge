...
- `make mock-eval` — all scripted scenarios offline via `evals/mock_llm.py` in ~5 s, zero API cost. Run after any dispatcher/flows/tools change. (Suite has grown beyond the original 16 — see `ARCHITECTURE.md` §11.)
...
- **Identity gate is a hard checkpoint.** Booking / cancel / reschedule tools are mounted only after `SessionMemory.identified_patient` is set. IDENTIFY_PATIENT exposes only `find_patient_by_phone` / `find_patient_by_name_dob`; `create_patient` lives in REGISTER_PATIENT (see `flows.py` `ALLOWED_TOOLS` and `ARCHITECTURE.md` §5).
...
- **`specialty` is a string filter on `list_availability_slots`.** Today the LLM picks the value from caller intent ("Therapist", "Psychiatrist", "General Practice", "Dermatologist", "Physiotherapist"). A mini-LLM complaint→specialty router is **in flight** — read `ARCHITECTURE.md` §14 before editing anything in `tools.py`, `flows.py`, or specialty-related scenarios so two agents don't conflict.
...
11. **`ARCHITECTURE.md` is the live ledger.** (Formerly `SOLUTION.md`; split 2026-05-25 — see below.) Anything shipped, removed, or moved out of in-flight must be reflected in `ARCHITECTURE.md` in the *same change* — update the relevant section and the §14 *In-flight work* list (drop finished items, add new ones). Code change without the matching `ARCHITECTURE.md` edit is incomplete. It is the canonical "what exists today"; if it lies, the next agent builds on a lie. **`SOLUTION.md` is now the non-technical executive overview** (audience: Prosper interviewers / leadership) — keep it in sync only when a *user-visible capability or headline number* changes, not on every internal wiring edit.
...
of the same change, exactly like rule 11 for `ARCHITECTURE.md`. A code change that
leaves its doc stale is incomplete.

| Doc | CONSULT when… | UPDATE when… |
|---|---|---|
| **`ARCHITECTURE.md`** ← start here (technical) | unsure how any feature is wired; before grepping; checking shipped-vs-in-flight (§14); how to run/operate | **any** feature shipped / removed / moved out of in-flight (rule 11) — same change, incl. §14 list |
| **`SOLUTION.md`** (exec overview) | want the non-technical pitch for leadership / interviewers | a user-visible capability or headline number (states / tools / scenario+test counts) changes |
...
| `docs/FEATURES.md` | want the exhaustive capability list (`ARCHITECTURE.md` §0.1 is the overview) | a user-visible capability ships or is removed |
...
| `docs/research/`, `docs/superpowers/specs/` | mining the reasoning behind a feature | **read-only history** — don't edit; may hold other agents' WIP. A finished spec's *outcome* belongs in `ARCHITECTURE.md`, not here |
