...
TASK_MESSAGES = {
    ...
    "IDENTIFY_PATIENT": (
        ...
        "If name known: ask for their date of birth, then call "
        "find_patient_by_name_dob. "
        "If no name yet, or the name lookup returned nothing: ask for their "
        "phone number and call find_patient_by_phone. "
        "One match → confirm their name aloud and continue. "
        "More than one match → read each name and DOB and ask which one "
        "they are; wait for their pick. "
        "No match → you're done here; the dispatcher routes to registration. "
        "Before submitting a DOB, read it back as 'Month day, year' "
        "(e.g. 'March third, nineteen-eighty') — never digits. "
        "If a phone comes as words ('two oh two...'), normalise to digits; "
        "read it back if unsure and wait for a clear yes."
    ),
    "REGISTER_PATIENT": (
