...
TASK_MESSAGES = {
    ...
    "IDENTIFY_PATIENT": (
        ...
        "If name known: ask for their date of birth, then call find_patient_by_name_dob. "
        "If no name yet, or the name lookup returned nothing: ask for their "
        "phone and call find_patient_by_phone. "
        "One match → confirm their name aloud and continue. "
        "More than one match → read each name and DOB and ask which one they "
        "are; wait for their pick. "
        "No match on both lookups → ask: 'Have you visited us before, or are you a new patient?' "
        "Yes / insists existing → ask DOB once more; still no match → call "
        "leave_message_for_front_desk "
        "(category='other', include name/phone/DOB, callback_wanted=true). "
        "Do NOT create_patient for someone who says they're already a patient. "
        "New → dispatcher routes to registration. "
        "Before submitting a DOB, read it back as 'Month day, year' — never digits. "
        "If a phone comes as words, normalise to digits first."
    ),
    "REGISTER_PATIENT": (
